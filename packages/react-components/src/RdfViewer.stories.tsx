...
const customLiteralRenderers: Record<string, LiteralRenderer> = {
  "http://www.w3.org/2001/XMLSchema#date": ({ literal }) => (
    <span className="literal date">Date: {literal.value}</span>
  ),
};

const customPredicateRenderers: Record<string, PredicateRenderer> = {
  "http://xmlns.com/foaf/0.1/knows": ({ defaultRender }) => (
    <span>
      {defaultRender()}
      <span className="content-type-hint"> related</span>
    </span>
  ),
};
...
export const NoLanguageTags: Story = {
  args: {
    data: foafPersonData,
